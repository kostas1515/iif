...
@LINEAR_LAYERS.register_module(name='IIFNormedLinear')
class IIFNormedLinear(nn.Linear):
    """IIF Normalized Linear Layer.
    ...
    def __init__(self, *args, tempearture=20, power=1.0, eps=1e-6,path='./lvis_files/idf_1204.csv',
                 variant='base2_obj', **kwargs):
        super(IIFNormedLinear, self).__init__(*args, **kwargs)
        self.tempearture = tempearture
        self.power = power
        self.eps = eps
        self.init_weights()
        ...
        self.iif_weights = torch.tensor(self.iif_weights,device='cuda',dtype=torch.float).unsqueeze(1)
    ...
    def forward(self, x):
        weight_ = (self.iif_weights*self.weight) / (
            (self.iif_weights*self.weight).norm(dim=1, keepdim=True).pow(self.power) + self.eps)
        x_ = x / (x.norm(dim=1, keepdim=True).pow(self.power) + self.eps)
        x_ = x_ * self.tempearture
